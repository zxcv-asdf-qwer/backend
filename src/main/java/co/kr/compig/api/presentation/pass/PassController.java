...
public class PassController {
	...
	public String getPassTestPage1(HttpServletRequest request, HttpServletResponse response,
		...
		if (getActiveProfile().contains("local")) {
			sReturnUrl = "http://localhost:8080/pass/test/success"; // 성공시 이동될 URL
			sErrorUrl = "http://localhost:8080/pass/test/fail"; // 실패시 이동될 URL
		}
		if (getActiveProfile().contains("dev")) {
			sReturnUrl = "https://api-dev.localhost.com/pass/test/success"; // 성공시 이동될 URL
			sErrorUrl = "https://api-dev.localhost.com/pass/test/fail"; // 실패시 이동될 URL
		}
