...
import co.kr.compig.global.code.converter.DomesticForeignCodeConverter;
...
public class Member {
	...
	@Column(length = 10)
	@Convert(converter = DomesticForeignCodeConverter.class)
   ...
   * Business
	...
	public void updateAdminMember(AdminMemberUpdate adminMemberUpdate) {
		...
		if (CollectionUtils.isNotEmpty(adminMemberUpdate.getGroupKeys())) {
			this.removeAllGroups(
				this.groups.stream()
					.filter(
						memberGroup ->
	...
	public void updatePartnerMember(PartnerMemberUpdate partnerMemberUpdate) {
		...
		if (CollectionUtils.isNotEmpty(partnerMemberUpdate.getGroupKeys())) {
			this.removeAllGroups(
				this.groups.stream()
					.filter(
						memberGroup ->
