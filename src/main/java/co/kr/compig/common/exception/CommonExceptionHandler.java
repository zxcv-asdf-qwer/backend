...
public class CommonExceptionHandler {
    ...
    protected ResponseEntity<ErrorResponse> handleGlobalException(final Exception e, final WebRequest er) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(ErrorResponse.of(e, er), HttpStatus.INTERNAL_SERVER_ERROR);
    }

    @ExceptionHandler(BrewException.class)
    protected ResponseEntity<ErrorResponse> handleBrewException(final BrewException e, final WebRequest er) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(ErrorResponse.of(e, er), e.getErrorCode().getHttpStatus());
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    protected ResponseEntity<ErrorResponse> customHandleMethodArgumentNotValid(MethodArgumentNotValidException ex, WebRequest request) {
        ...
        List<ErrorResponse.FieldError> fieldErrors = ex.getBindingResult().getGlobalErrors()
                .stream()
                .map(objectError -> new ErrorResponse.FieldError(objectError.getObjectName(), objectError.getDefaultMessage())).toList();
    ...
    protected ResponseEntity<ErrorResponse> handleTypeMismatch(TypeMismatchException ex, WebRequest request) {
